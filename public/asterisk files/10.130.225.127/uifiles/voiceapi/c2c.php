...
if (isset($_REQUEST['agent'])) {
    $agent = mysql_real_escape_string(filter_input(INPUT_GET, 'agent', FILTER_SANITIZE_SPECIAL_CHARS));
}
...
$agent_number = ($agent==0)?$campaign_arr['agent_number']:$agent;
formatData($msisdn, $campaignid, $user_id, $campaign_arr['retry'], $campaign_arr['clip_name'], $campaign_arr['templateid'], $agent_number);
$success_arr = Array(
    "campaignid" => $campaignid,
    "code" => "100",
    "ts" => Date('Y-m-d H:i:s'),
...
function get_campaignDetails($user_id, $campaignid) {
    $camp = Array();
    $camp_qry = "select retry,audio_clip,template_id,agent_number from campaigns where client_id = '$user_id' and id = $campaignid and agent_number is not null LIMIT 1";
...
function formatData($msisdn, $camp_id, $user_id, $retry, $clip_name, $template_id, $agent_number) {
    $clip_name = substr($clip_name, 0, -4) . '.wav';
    //$csv_arr = array_map('str_getcsv', file($csv_uploadPath . $csv_filename));
    $insert_obd_data = "insert into subscribers(msisdn,audio_clip,campid,user_id,retry,sch_time,process_state,template_id, agent_number) "
            . "values ('$msisdn','$clip_name', $camp_id, $user_id, $retry, NOW(), 1, $template_id, '$agent_number')";
